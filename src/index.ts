import { ExtractFeatureCoreFunctionArgs, ExtractionParams } from 'feature-extractor-worker';
...
async function askWorkerToExtractFeatures(buffers: Float32Array[], audioFeatures: MeydaAudioFeature[], {bufferSize, hopSize, zeroPadding, windowingFunction}: Partial<ExtractionParams>): Promise<DataObj>{
...
async function getAudioChannelDataFromBlob(audioBlob: Blob, selectedChannels? :number[]): Promise<Float32Array[]> {
  const audioBuffer = await getAudioBufferFromBlob(audioBlob);
  const defaultChannels = Array.from({length: audioBuffer.numberOfChannels}, (_,i) => i);;
  return (selectedChannels || defaultChannels).map(channelIndex => audioBuffer.getChannelData(channelIndex));
}

function extractFeatureFunctionFactory(extractFeatureCoreFunction: (buffers: Float32Array[], audioFeatures: MeydaAudioFeature[], extractionParams: Partial<ExtractionParams>) => Promise<any>) {
